...
app.use(function(req,res,next) {
 ...
 // update the template with the user level and verify if the editor
  req.TPL.user_level = req.session.user_level;
  req.TPL.is_editor = (req.session.user_level === 'editor');
  next();
});
...
// ensure that only editors have access to the editors page.
app.use("/editors", function(req,res,next) {
  if (req.session.user_level === 'editor') next();
  else res.redirect("/home");
});
// Include Controllers
//
...
// Start the server
var server = app.listen(3000, function() {console.log("Server listening to http://localhost:3000");})